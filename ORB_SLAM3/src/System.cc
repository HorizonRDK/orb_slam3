...
System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false), mthreadPool(nullptr)
{
...
void System::CreateTrackFrameThread() {
  auto TrackFrame = [this](){
    ...
    while (!mbfinished) {
      ...
      mQueueCond.wait(lck, [&]() {
        return (!mFrameQueue.empty() && mFrameQueue.begin()->second != nullptr)
              || mbfinished;
      });
...
std::future<Sophus::SE3f> System::TrackStereoAsync(
  ...
  if (mSensor != STEREO && mSensor != IMU_STEREO) {
    std::cerr << "ERROR: you called TrackStereo "
                 ...
                 "not set to Stereo nor Stereo-Inertial." << std::endl;
    exit(-1);
  }
  ...
  mthreadPool->PostTask(std::bind(&System::CreateFrameAndPush, this,
          imLeft, imRight, timestamp, filename, vImuMeas, pose_promise));
