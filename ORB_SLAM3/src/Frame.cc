...
void Frame::ComputeStereoMatches()
{
    ...
    for(int iL=0; iL<N; iL++)
    {
        ...
        if(bestDist<thOrbDist)
        {
            ...
            const int w = 5;
            int startvrow = scaledvL - w;
            int endvrow = scaledvL + w + 1;
            int startucol = scaleduL - w;
            int enducol = scaleduL + w + 1;
            if (startvrow < 0 || startvrow > endvrow
              || endvrow >  mpORBextractorLeft->mvImagePyramid[kpL.octave].rows
              || startucol < 0 || startucol > enducol) {
              continue;
            }
            cv::Mat IL = mpORBextractorLeft->mvImagePyramid[kpL.octave].rowRange(
                    startvrow, endvrow).colRange(startucol, enducol);
            ...
            {
              int startcol = scaleduR0 + incR - w;
              int endcol = scaleduR0 + incR + w + 1;
              auto right_mat_rowrange = mpORBextractorRight->mvImagePyramid[kpL.octave].rowRange(
                      startvrow, endvrow);
              if (startcol < 0 || startcol > endcol || endcol > right_mat_rowrange.cols) {
//                printf("error!!! startcol:%d, endcol:%d, cols:%d \n",
//                        startcol, endcol, right_mat_rowrange.cols);
//                printf("scaleduR0: %f, incR: %d, w: %d\n", scaleduR0, incR, w);
//                printf("uR0: %f, scaleFactor: %f\n", uR0, scaleFactor);
                continue;
              }
              cv::Mat IR = right_mat_rowrange.colRange(startcol, endcol);
