...
void ImageGrabber::GrabStereo(const sensor_msgs::msg::Image::ConstSharedPtr msgLeft,const sensor_msgs::msg::Image::ConstSharedPtr msgRight)
{
    ...
    if(!mpImuGb->imuBuf.empty())
    {
        ...
        vImuMeas.clear();
        while(!mpImuGb->imuBuf.empty() && rclcpp::Time(mpImuGb->imuBuf.front()->header.stamp).seconds()<=tImLeft)
        {
...
int main(int argc, char **argv)
{
    ...
    if(argc!=3)
    {
        cerr << endl << "Usage: rosrun ORB_SLAM3 Stereo_Inertial path_to_vocabulary path_to_settings " << endl;
        rclcpp::shutdown();
        return 1;
    }
    ...
    message_filters::Subscriber<sensor_msgs::msg::Image> left_sub(igb.node_,"/cam0/image_raw",rclcpp::SensorDataQoS().get_rmw_qos_profile());
    message_filters::Subscriber<sensor_msgs::msg::Image> right_sub(igb.node_,"/cam1/image_raw",rclcpp::SensorDataQoS().get_rmw_qos_profile());
